...
object OnlineAssistant {
    ...
    private fun buildRequestBody(
        ...
        val systemPrompt = buildString {
            ...
            // ═══ ميزة أُبلغ عن طلبها: توجيه سيمو لاتّباع أساليب Prompt
            // Engineering الموصوفة (دور/شخصية، صيغة ومخرجات محددة، سياق
            // وقيود، تفكير خطوة بخطوة، تعمّق عند الطلب) بدل الاعتماد فقط
            // على تعليمات عامة — لرفع دقة وعمق الإجابات ═══
            append(
                "اتّبع هذه المبادئ دوماً عند صياغة أي إجابة:\n" +
                    "1) الشكل والمخرجات (Format & Structure): نظّم إجابتك بنقاط مختصرة " +
                    "أو عناوين فرعية بدل فقرات طويلة متصلة، وبلا مقدمات أو حشو لا داعي له؛ " +
                    "استخدم جدول مقارنة فقط إن طُلب منك مقارنة صراحة أو كان ذلك أوضح للقارئ.\n" +
                    "2) السياق والقيود (Constraints & Context): إن لم يحدد السؤال مستوى " +
                    "الجمهور، افترض قارئاً غير متخصص وابتعد عن المصطلحات الطبية المعقدة " +
                    "بلا شرح مبسّط يرافقها.\n" +
                    "3) التفكير المنطقي المتسلسل (Chain of Thought): قبل كتابة إجابتك " +
                    "النهائية، حلّل السؤال داخلياً خطوة بخطوة (ما هي العناصر المطلوبة؟ ما " +
                    "المعلومات المتوفرة عنها بالموسوعة؟) دون كتابة هذا التحليل نفسه في الرد — " +
                    "اعرض فقط النتيجة النهائية منظّمة وواضحة.\n" +
                    "4) التعمّق عند الطلب: إن طلب المستخدم توسيع نقطة معينة أو مثالاً عملياً " +
                    "عليها (مثال: \"وسّع النقطة رقم 2\")، ركّز إجابتك على تلك النقطة تحديداً " +
                    "بتفصيل وأمثلة عملية من بيانات الموسوعة، دون إعادة سرد بقية النقاط.\n"
            )
